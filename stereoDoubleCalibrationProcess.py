...
def applyNavTransform(inputCubePath, outputCubePath, transformMatrixPath, workDir, 
    ...
    cmd = ('rotationCorrector.py --keep --input ' + inputCubePath + ' --output ' + outputCubePath + 
          ' --transformPath ' + transformMatrixPath + ' --workDir ' + workDir + ckLine + spkLine)
    print cmd
    os.system(cmd)
...
def main():
    ...
    try:
        ...
        filename                 = os.path.splitext(options.leftPath)[0] + '.lronaccal.lronacecho.cub'
        spiceInitLeftPath        = os.path.join(tempFolder, os.path.basename(filename))
        filename                 = os.path.splitext(options.rightPath)[0] + '.lronaccal.lronacecho.cub'
        spiceInitRightPath       = os.path.join(tempFolder, os.path.basename(filename))
        filename                 = os.path.splitext(options.stereoLeft)[0] + '.lronaccal.lronacecho.cub'
        spiceInitStereoLeftPath  = os.path.join(tempFolder, os.path.basename(filename))
        filename                 = os.path.splitext(options.stereoRight)[0] + '.lronaccal.lronacecho.cub'
        spiceInitStereoRightPath = os.path.join(tempFolder, os.path.basename(filename))
