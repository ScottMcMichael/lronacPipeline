...
def main():
    ...
    try:
        ...
        # Extract a small number of matching pixel locations ( < 300)
        pixelPairsSmall = os.path.join(tempFolder, 'stereoPixelPairsSmall.csv')
        if not os.path.exists(pixelPairsSmall):
            cmd = '/home/smcmich1/repo/lronacPipelineBuild/pixelPairsFromStereo -i ' + disparityImage + ' -o ' + pixelPairsSmall + ' -p 800'
            print cmd
            os.system(cmd)
        else:
        ...
        if True:#not os.path.exists(globalParamsFile):
            cmd = '/home/smcmich1/repo/lronacPipelineBuild/lronacAngleSolver --outputPath ' + globalParamsFile + ' --gdcPointsOutPath ' + smallGdcFile + ' --matchingPixelsPath ' + pixelPairsSmall + ' ' + posOffsetCorrectedLeftPath + ' ' + posOffsetCorrectedStereoLeftPath + ' --worldTransform --includePosition'
            print cmd
            os.system(cmd)
        else:
        ...
        # With Ceres solver and 335 points, -3.7 error
        # TODO: Compute median error, a few bad points are skewing the average
        ...
        if not os.path.exists(pixelPairsLarge):
            cmd = '/home/smcmich1/repo/lronacPipelineBuild/pixelPairsFromStereo -i ' + disparityImage + ' -o ' + pixelPairsLarge + ' -p 10'
            print cmd
            os.system(cmd)
        else:
...
#       Also maybe skip the unload step.
        ...
        if True:#not os.path.exists(pairGdcCheckPath):
            cmd = '/home/smcmich1/repo/lronacPipelineBuild/lronacAngleSolver --outputPath dummy.txt --gdcPointsOutPath ' + pairGdcCheckPath + ' ' + options.outputPathLeft + ' ' + tempRightPath
            print cmd
            os.system(cmd)
